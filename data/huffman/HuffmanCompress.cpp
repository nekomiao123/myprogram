...
int main(){
	...
	FILE *fpr = fopen("text.txt","r");//读入文件，其中input.txt的路径你要自己设置，可以自己建立一个文本，写一些英文进去 
	FILE *fpw = fopen("output1","wb");//写入文件，wb是写入二进制文件，路径设置随意，但是要符合格式，写入的时候文件自动生成。
	FILE *fpr1= fopen("output1","rb");;//用于解压缩时读入文件 ，rb是读入二进制文件 
	FILE *fpw1 = fopen("output3.txt","w"); //用于解压缩时写入文件 
	//解压需要用的 
	char op;
	haffNode *z;
	//读取文件 
